export class CanvasLocal {
    ...
    drawHexfaceTapa(x1, y1, width, height, r, g, b) {
        let unidad = width;
        this.drawRomboide(x1, y1 + height, x1 + width * 0.5, y1 + height - unidad * -0.2, x1 + width * 0.5, y1 - unidad * 0.2, x1, y1, "rgb(" + (r - 50) + "," + (g + 40) + "," + b + ")");
        //this.drawRomboide(x1, y1-width, x1 + height *0.2, y1 + height - unidad*0.5, x1 + width *0.2, y1 - unidad*0.5, x1, y1+height, "rgb(" + r + "," + (g+225) + "," + b + ")");
        this.drawRomboide(x1, y1 + height, x1 - width * 0.5, y1 + height - unidad * -0.2, x1 - width * 0.5, y1 - unidad * 0.2, x1, y1, "rgb(" + (r - 50) + "," + (g + 40) + "," + b + ")");
        this.drawRomboide(x1 + width * 0.5, y1 + height - unidad * -0.2, x1 + width * 0.75, y1 + height - unidad * -0.5, x1 + width * 0.75, y1 - unidad * 0.5, x1 + width * 0.5, y1 - unidad * 0.2, "rgb(" + (r - 50) + "," + (g + 40) + "," + b + ")");
        this.drawRomboide(x1 - width * 0.5, y1 + height - unidad * -0.2, x1 - width * 0.75, y1 + height - unidad * -0.5, x1 - width * 0.75, y1 - unidad * 0.5, x1 - width * 0.5, y1 - unidad * 0.2, "rgb(" + (r - 50) + "," + (g + 40) + "," + b + ")");
    }
    paint() {
        ...
        // this.drawLine(this.iX(0), this.iY(0), this.iX(0), this.iY(6));
        //this.drawLine(this.iX(2), this.iY(0), this.iX(2), this.iY(2));
        this.graphics.strokeStyle = 'black';
        let ind = 0;
        let r = 255;
        let g = 0;
        ...
        // this.iY(6 * h[ind] / maxEsc) - this.iY(0), r, g, b);
        //this.drawHexfaceTapa(this.iX(i), this.iY(6 * h[ind] / maxEsc), this.iX(1) - this.iX(0),
        //this.iY(6 * h[ind] / maxEsc) - this.iY(0), r, g, b);
        for (let i = 1; i <= 8; i += 2) {
            this.drawHexface(this.iX(i), this.iY(6 * h[ind] / maxEsc), this.iX(1) - this.iX(0), this.iY(6 * h[ind] / maxEsc) - this.iY(0), r, g, b);
            this.drawHexfaceTapa(this.iX(i), this.iY(6 * h[ind] / maxEsc - 0.2), this.iX(1) - this.iX(0), this.iY(6 * h[ind] / maxEsc - 0.1) - this.iY(0), r, g, b);
            // this.drawRomboide(this.iX(i + 1), this.iY(6 * h[ind] / maxEsc-0.1), this.iX(i + 1.3), this.iY(6 * h[ind] / maxEsc + 0.2),
        ...
        /*for (let i = 0.5; i <=8; i += 2){
          ...
          this.drawRomboide(this.iX(i + 0.3), this.iY(6 * h[ind] / maxEsc + 0.2), this.iX(i + 1.3), this.iY(6 * h[ind] / maxEsc + 0.2),
